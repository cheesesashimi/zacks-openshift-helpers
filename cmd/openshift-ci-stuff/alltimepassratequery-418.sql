WITH passfail AS (
    ...
    FROM junits
    WHERE
        (test_name LIKE '%ocb%' OR test_name LIKE '%layering%' OR test_name LIKE '%ocl%' AND test_name NOT LIKE '%OnCLayer%') AND result IN('passed', 'failed') AND job_name LIKE '%4.18%'
    GROUP BY test_name
)
