WITH passfail AS (
    ...
    FROM junits
    WHERE
        (test_name LIKE '%ocb%' OR test_name LIKE '%layering%' OR test_name LIKE '%ocl%' AND test_name NOT LIKE '%OnCLayer%') AND result IN('passed', 'failed') AND DATE(started) BETWEEN date('now', '-7 day') AND date() AND job_name LIKE '%4.19%'
    GROUP BY test_name
)
