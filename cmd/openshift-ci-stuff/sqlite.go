...
func importIntoSqlite(results jobTestResults) error {
	...
	_, err := os.Stat(junitsDBName)
	if err == nil {
		fmt.Println("Found preexisting sqlite database, deleting first...")
		if err := os.RemoveAll(junitsDBName); err != nil {
			return err
		}
	...
	db, err := sql.Open("sqlite3", junitsDBName)
	if err != nil {
		return err
	}
	defer db.Close()
...
//go:embed weeklypassratequery-all.sql
var weeklyPassRateQueryAll string

//go:embed weeklypassratequery-418.sql
var weeklyPassRateQuery418 string

//go:embed weeklypassratequery-419.sql
var weeklyPassRateQuery419 string
...
// by name.
func getWeeklyPassRatesFromSqlite(query string) (map[string][]passRateRow, map[string]aggregatedResult, error) {
	db, err := sql.Open("sqlite3", junitsDBName)
	if err != nil {
		return nil, nil, err
	}
	...
	for rows.Next() {
		prr := passRateRow{}
		var weekStr string
		err = rows.Scan(&prr.testName, &weekStr, &prr.passed, &prr.failed, &prr.total, &prr.passRate)
		if err != nil {
			return nil, nil, err
		}
...
// rates.
func refreshDataAndProduceWeeklyPassRateCSVReport() error {

	if err := createSqliteDatabaseFromCISystem(); err != nil {
		return err
	}

	if err := produceWeeklyPassRateCSVReports(); err != nil {
		return err
	}
...
func produceWeeklyPassRateCSVReports() error {
	queries := map[string]string{
		"all":  weeklyPassRateQueryAll,
		"4.18": weeklyPassRateQuery418,
		"4.19": weeklyPassRateQuery419,
	}

	for ocpVersion, query := range queries {
		filename := fmt.Sprintf("passrates-%s.csv", ocpVersion)
		if err := produceWeeklyPassRateCSVReport(filename, query); err != nil {
			return fmt.Errorf("could not produce weekly pass rate CSV report for %s: %w", filename, err)
		}
	}
...
func produceWeeklyPassRateCSVReport(filename, query string) error {
	byName, totals, err := getWeeklyPassRatesFromSqlite(query)
	if err != nil {
		return err
	}
