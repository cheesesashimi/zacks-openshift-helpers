...
func findAndParseExtendedLogs() error {
	return filepath.Walk(workspaceRoot, func(path string, info fs.FileInfo, err error) error {
		if err != nil {
			return err
		}

		if info.Name() == "extended.log" {
			return parseExtendedLog(path)
		}
